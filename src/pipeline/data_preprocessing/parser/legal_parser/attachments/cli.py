    ...
    *,
    ...
    for file_path in files:
        try:
            ...
            log.info(
                "OK %s | kind=%s | nodes=%s | tables=%s | tree=%s",
                file_path.name,
                kind.kind,
                parsed.get("node_count"),
                parsed.get("table_count"),
                parsed.get("tree_path"),
            )
        except Exception as exc:
            errors.append((str(file_path), str(exc)))
            log.error("Loi attachment %s: %s", file_path, exc)

    inventory["aggregate_counts"] = {
        "attachments": len(parsed_dirs),
        "nodes": sum((item.get("node_count") or 0) for item in inventory["attachments"]),
        "tables": sum((item.get("table_count") or 0) for item in inventory["attachments"]),
    }
    if errors:
    ...
    log.info(
        "Hoan tat attachments | success=%s/%s | nodes=%s | tables=%s",
        len(parsed_dirs),
        len(files),
        inventory["aggregate_counts"]["nodes"],
        inventory["aggregate_counts"]["tables"],
    )
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Parse Vietnamese legal attachments DOC/DOCX into tree JSON.")
