...
from .utils import collapse_ws, ensure_dir, extract_all_vietnamese_dates, md5_text, normalize_document_number, normalize_id, parse_vietnamese_date_to_iso, read_jsonl, units_file_output_name
...
class EffectivityExtractor:
    ...
    def extract_from_units_file(self, units_path: Union[str, Path], output_dir: Optional[Union[str, Path]] = None) -> Dict[str, Any]:
        ...
        effectivity = {
            "general": self.build_general_effectivity(units, events),
            "units": self.build_unit_effectivity(events),
        }
        with open(output_dir / "effectivity.json", "w", encoding="utf-8") as f:
            json.dump(effectivity, f, ensure_ascii=False, indent=2)
        return effectivity

    def build_general_effectivity(self, units: List[Dict[str, Any]], events: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        rows: Dict[str, Dict[str, Any]] = {}

        if units:
            first_unit = units[0]
            doc_id = first_unit.get("document_id") or "unknown"
        ...
        return self._clean_general_rows(rows.values())

    @staticmethod
    def build_unit_effectivity(events: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        rows = []
        seen = set()
        for ev in events:
    ...
    @staticmethod
    def _clean_general_rows(rows: Any) -> List[Dict[str, Any]]:
        cleaned = []
        for row in rows:
            cleaned.append({
                ...
                "effective_to_source_document_number": row.get("effective_to_source_document_number"),
                "in_corpus": bool(row.get("in_corpus")),
            })
        return sorted(cleaned, key=lambda item: (not item["in_corpus"], item.get("document_id") or ""))

    @classmethod
    def merge_general_effectivity(cls, rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        merged: Dict[str, Dict[str, Any]] = {}
        for row in rows:
            key = cls._document_key(row.get("document_number"), row.get("document_id"))
            ...
            current["in_corpus"] = bool(current.get("in_corpus") or row.get("in_corpus"))
            if row.get("document_id") and current.get("document_id") in {None, "unknown"}:
                current["document_id"] = row.get("document_id")
            if row.get("document_number") and not current.get("document_number"):
                current["document_number"] = row.get("document_number")
            if row.get("effective_from") and (
                not current.get("effective_from") or row["effective_from"] < current["effective_from"]
            ):
                current["effective_from"] = row["effective_from"]
            if row.get("effective_to") and (
                not current.get("effective_to") or row["effective_to"] < current["effective_to"]
            ):
                current["effective_to"] = row["effective_to"]
                current["effective_to_source_document_number"] = row.get("effective_to_source_document_number")
        return cls._clean_general_rows(merged.values())
