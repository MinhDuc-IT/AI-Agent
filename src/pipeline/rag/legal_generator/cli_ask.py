...
from legal_rag.qdrant_settings import apply_qdrant_settings, qdrant_target_label
...
def main() -> None:
    ...
    print(f"Qdrant target: {qdrant_target_label(retriever_config)}")
    print(f"LLM model: {generator_config.model}")
    result = LegalQAGenerator(retriever_config, generator_config).ask(
        args.query,
